...
for(g in groups) {
  ...
  map <- map %>% addCircles(data = d,
                            ...
                            group = g,
                            color = ~pal(individual.local.identifier), 
                            popup = paste( # we can use HTML to format a popup which is accessed when points are clicked
...
# we can toggle the seal paths we want to see using the 'overlayGroups' attribute in addLayersControl()
